...
def seo():
    ...
    if not kw:
        kw = my['tags'][0] if my.get('tags') else '힐링'
    
    # SEO 제목 공식: [숫자+단위] 키워드 - 효과 이모지
    # 예: [10분] 강아지 수면음악 - 꿀잠 보장 🐶
    formulas = [
        f"[1시간] {kw} - 깊은 숙면 보장 💤",
        f"[무한반복] {kw} 테라피 - 스트레스 해소 ✨",
        f"[10분] {kw} 마법 - 1분 만에 잠드는 사운드 🌙"
    ]
    
    sets = []
    import random
    tags_list = my.get('tags', ['힐링', '숙면', kw])
    for i, title in enumerate(formulas):
        # 태그 중 무작위 5개 선택 + 키워드
        random_tags = random.sample(tags_list, min(len(tags_list), 5))
        if kw not in random_tags: random_tags.append(kw)
        
        sets.append({
            "title": title,
            "desc": f"{my['title']} 채널에서 제공하는 {kw} 전문 사운드입니다. {random_tags[0]} 효과로 마음의 안정을 찾으세요. # {kw} #{random_tags[1]}",
            "tags": ",".join(random_tags)
        })
    
    session['seo_data'] = sets
    return jsonify(sets)
...
def generate_video():
    ...
    try:
        ...
        img.save(bg)
        # 정지 이미지 기반 5초 영상 생성
        subprocess.run(["ffmpeg", "-y", "-loop", "1", "-i", bg, "-f", "lavfi", "-i", "anullsrc", "-c:v", "libx264", "-t", "5", "-pix_fmt", "yuv420p", "-c:a", "aac", "-shortest", out], capture_output=True)
    except Exception as e:
        # PIL 실패 시 검정 배경으로 fallback
        subprocess.run(["ffmpeg", "-y", "-f", "lavfi", "-i", "color=c=black:s=1280x720:d=5", "-pix_fmt", "yuv420p", out], capture_output=True)
    
    if os.path.exists(out):
        return jsonify({"status": "success", "output_path": out})
    return jsonify({"error": "영상 제작 실패"}), 500
