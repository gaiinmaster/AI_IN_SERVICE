...
def generate_plan(prompt, file_path):
    ...
    print(f"Generating full detailed plan for {file_path}...")
    full_prompt = f"아래 요구사항에 맞춰서 마크다운 형식의 '본문 내용만' 출력하세요. 다른 설명은 하지 마세요.\n\n{prompt}"
    try:
        res = subprocess.run(['gemini', '-y', full_prompt], capture_output=True, text=True, timeout=600)
        content = res.stdout.strip()
        
        # Remove any leading/trailing meta-talk if gemini still adds it
        # This is hard to do perfectly, but usually gemini -y with such prompt works.
...
# Detailed Prompts
naver_mcp_prompt = """
# 프로젝트 기획안: Naver MCP Server (K-Business Localization)
...
daeri_app_prompt = """
# 프로젝트 기획안: 혁신적 대리운전 플랫폼 '오지마(Ozima)'
...
4. 수익 모델 상세 시뮬레이션:
   - 수수료 10% vs 월구독 5만원 수익성 비교 (기사 1,000명 / 5,000명 / 10,000명 기준).
